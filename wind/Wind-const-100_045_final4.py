...
# Diagnose: Mehrere reale Zementwerke in derselben NUTS-2-Region
# werden im Modell und Plot bewusst zu EINEM Zementpunkt aggregiert.
dup_cement_nuts = zement_raw["NUTS_ID"].value_counts()
multi_cement_nuts = dup_cement_nuts[dup_cement_nuts > 1]
if not multi_cement_nuts.empty:
    print("⚠️ Mehrere Zementwerke liegen in derselben NUTS-2-Region und werden im Modell/Plot auf einen Punkt aggregiert:")
    print(multi_cement_nuts.to_string())
...
T_count = len(T)
# 2. Mapping: Modell-Jahr (int) -> Excel-Spaltenname (str)
# WICHTIG: Diese Namen müssen EXAKT so in Ihrer neuen Excel-Datei stehen!
year_map = {
    2026: "2026-2030",
    2031: "2031-2035",
    2036: "2036-2040",
...
# 1) Supply-Standorte
supply_gdf_proj.loc[220, "iso"] = "UK"    
supply_gdf_proj.loc[221, "iso"] = "UK"  
supply_gdf_proj.loc[222, "iso"] = "UK"  
supply_gdf_proj.loc[223, "iso"] = "UK"  
supply_gdf_proj.loc[224, "iso"] = "UK"  
...
# =============================================================================
# DIAGNOSE: Warum fehlen Verbindungen?
# =============================================================================

print("--- DIAGNOSE START: Abgleich mit Persyn-Matrix ---")

# 1. Verfügbare IDs in der Matrix (Zeilen und Spalten)
persyn_rows = set(cost_matrix_persyn.index)
persyn_cols = set(cost_matrix_persyn.columns)

# 2. Welche Ihrer Angebotsorte (A) fehlen in den Persyn-ZEILEN?
# (Weil Transport A -> R bedeutet: A ist Start/Zeile)
missing_A = [a for a in A if a not in persyn_rows]

# 3. Welche Ihrer Recyclingorte (R) fehlen in den Persyn-SPALTEN?
# (Weil Transport A -> R bedeutet: R ist Ziel/Spalte)
missing_R_as_dest = [r for r in R if r not in persyn_cols]

# 4. Welche Ihrer Recyclingorte (R) fehlen in den Persyn-ZEILEN?
# (Weil Transport R -> Z bedeutet: R ist Start/Zeile)
missing_R_as_start = [r for r in R if r not in persyn_rows]

# 5. Welche Ihrer Zement-Regionen (Z) fehlen in den Persyn-SPALTEN?
missing_Z = [z for z in Z if z not in persyn_cols]

print(f"\nAnzahl fehlende A (als Start): {len(missing_A)}")
if len(missing_A) > 0:
    print(f"Beispiele fehlende A: {missing_A[:10]}")

print(f"\nAnzahl fehlende R (als Ziel): {len(missing_R_as_dest)}")
if len(missing_R_as_dest) > 0:
    print(f"Beispiele fehlende R (Ziel): {missing_R_as_dest[:10]}")

print(f"\nAnzahl fehlende R (als Start): {len(missing_R_as_start)}")
# Wenn R als Start fehlt, aber als Ziel da ist, ist die Matrix asymmetrisch

print(f"\nAnzahl fehlende Z (als Ziel): {len(missing_Z)}")
if len(missing_Z) > 0:
    print(f"Beispiele fehlende Z: {missing_Z[:10]}")
...
for r in R:                       # alle Recyclingwerke
    for t in T:                   # alle Jahre
        mip.add_constraint(
            mip.sum(x_ar_g[a, r, "groß", t]           # <-- nur groß
                     for a in A) 
            <= b_rt[r, t] * K_r_groß
        )
...
print("Erstelle Zielfunktion (Szenario: Nur Wind)...")
...
ax.set_title(f"Abfallaufkommen (Vergleichbar): 2026-2050")
ax.axis("off")
plt.tight_layout()
plt.show()
...
years = [i for i in range(2026, 2051, 5)]
xlim  = (2.5e6, 6.1e6)
ylim  = (1.3e6, 5.5e6)
